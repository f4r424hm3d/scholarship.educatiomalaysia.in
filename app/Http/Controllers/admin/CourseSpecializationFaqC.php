...
class CourseSpecializationFaqC extends Controller
{
  protected $page_route;
  public function __construct()
  {
    $this->page_route = 'course-specialization-faqs';
  }
  public function index(Request $request, $specialization_id, $id = null)
  {
    ...
    $page_no = $_GET['page'] ?? 1;
    $rows = CourseSpecializationFaq::where('specialization_id', $specialization_id)->get();
    if ($id != null) {
      $sd = CourseSpecializationFaq::find($id);
      if (!is_null($sd)) {
        $ft = 'edit';
  ...
  public function store(Request $request)
  {
    ...
    $field = new CourseSpecializationFaq();
    $field->specialization_id = $request['specialization_id'];
    $field->sid = $request['specialization_id'];
  ...
  {
    if ($id) {
      $row = CourseSpecializationFaq::findOrFail($id);
  ...
  public function update($specialization_id, $id, Request $request)
  {
    ...
    $field = CourseSpecializationFaq::find($id);
    $field->specialization_id = $request['specialization_id'];
    $field->question = $request['question'];
    $field->answer = $request['answer'];
    $field->save();
  ...
  public function getData(Request $request)
  {
    // return $request;
    // die;
    $rows = CourseSpecializationFaq::where('specialization_id', $request->specialization_id)->paginate(10)->withPath('/admin/' . $this->page_route . '/' . $request->specialization_id);
    $i = 1;
